...
const originalFetch = (global as unknown as { fetch?: typeof global.fetch })
  .fetch;
...
describe("BazaarClient — construction", () => {
  ...
  it('network par défaut est "skale"', () => {
    const client = new BazaarClient({ privateKey: TEST_PRIVATE_KEY });
    assert.equal(client.network, "skale");
  });
...
// ─── searchServices ───────────────────────────────────────────────────────────
// searchServices délègue au backend GET /search?q=... (Gemini AI smart search).
// Le mock simule une réponse backend déjà filtrée — le SDK ne filtre plus côté client.

describe("BazaarClient.searchServices()", () => {
  it("appelle GET /search?q=... et retourne les résultats backend (array direct)", async () => {
    const client = createClient(BASE_CONFIG);
    const weatherFixture = [FIXTURE_SERVICES[1]!]; // uuid-weather

    setFetch(
      makeFetch([{ status: 200, ok: true, json: async () => weatherFixture }]),
    );
      ...
      makeFetch([
        {
          status: 200,
          ok: true,
          json: async () => ({ data: [FIXTURE_SERVICES[0]!] }),
        },
      ]),
    );
  ...
  it("gère une réponse enveloppée dans { services: [...] }", async () => {
    const client = createClient(BASE_CONFIG);
      ...
      makeFetch([
        {
          status: 200,
          ok: true,
          json: async () => ({ services: [FIXTURE_SERVICES[2]!] }),
        },
      ]),
    );
  ...
  it("gère une réponse enveloppée dans { results: [...] }", async () => {
    const client = createClient(BASE_CONFIG);
      ...
      makeFetch([
        {
          status: 200,
          ok: true,
          json: async () => ({ results: [FIXTURE_SERVICES[2]!] }),
        },
      ]),
    );

    try {
      const results = await client.searchServices("image");
      assert.equal(results.length, 1);
      assert.equal(results[0]!.id, "uuid-image");
    } finally {
      restoreFetch();
  ...
  it("retourne tous les services si query vide (fallback listServices)", async () => {
    const client = createClient(BASE_CONFIG);
  ...
  it("retourne tableau vide si le backend renvoie un tableau vide", async () => {
    const client = createClient(BASE_CONFIG);

    setFetch(makeFetch([{ status: 200, ok: true, json: async () => [] }]));
...
describe("BazaarClient.getService()", () => {
  it("retourne le détail d'un service par ID", async () => {
    ...
    setFetch(makeFetch([{ status: 200, ok: true, json: async () => fixture }]));
...
describe("BazaarClient.fundWallet()", () => {
  ...
  it("tous les champs requis sont présents et non vides", async () => {
    ...
    assert.ok(
      Array.isArray(info.supportedChains) && info.supportedChains.length > 0,
      "supportedChains non vide",
    );
...
describe("loadOrCreateWallet() — génération et persistance", () => {
  ...
  function tmpWalletPath(): string {
    return path.join(
      os.tmpdir(),
      `x402-sdk-test-wallet-${Date.now()}-${Math.random().toString(36).slice(2)}.json`,
    );
  }
  ...
  it("génère un nouveau wallet si le fichier n'existe pas", () => {
    ...
    try {
      ...
      assert.equal(
        info.address.length,
        42,
        "adresse Ethereum de 42 caractères",
      );
    } finally {
...
describe("PaymentHandler.sendViaFacilitator()", () => {
  ...
  const FEE_SPLITTER =
    "0x820d4b07D09e5E07598464E6E36cB12561e0Ba56" as `0x${string}`;
  const FAKE_TX =
  ...
  it("lève PaymentError si le réseau n'est pas polygon", async () => {
    ...
    await assert.rejects(
      ...
      (err: unknown) => {
        ...
        assert.ok(
          (err as PaymentError).message.toLowerCase().includes("polygon"),
        );
        return true;
      },
    );
  ...
  it("retourne un PaymentResult avec txHash si le facilitateur répond success", async () => {
    ...
    type CapturedSettle = {
      ...
      paymentPayload: {
        network: string;
        payload: { signature: string; authorization: unknown };
      };
    ...
    try {
      const result = await handler.sendViaFacilitator(
        RECIPIENT,
        0.005,
        FACILITATOR_URL,
      );
      ...
      assert.ok(
        (body.paymentPayload.payload.signature as string).startsWith("0x"),
      );
    } finally {
      restoreFetch();
    }
  ...
  it("accepte feeSplitterContract et l'utilise comme destinataire de l'autorisation", async () => {
    ...
    try {
      ...
      assert.ok(
        ...
        "authorization doit être présente dans le payload",
      );
...
describe("BazaarClient — auto-wallet (sans privateKey)", () => {
  function tmpWalletPath(): string {
    return path.join(
      os.tmpdir(),
      `x402-sdk-client-wallet-${Date.now()}-${Math.random().toString(36).slice(2)}.json`,
    );
  }
